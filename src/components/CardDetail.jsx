    ...
    return (
        <div className="card bg-base-200">
            <div className="card-body p-1">
                <h2 className="text-center">{itinerary.name}</h2>
                <p className="text-center">Name User: {itinerary.user}</p>
                <p className="text-center">Image User: {itinerary.user}</p>
                <p className='flex flex-wrap'>Price:
                    {Array.from({ length: itinerary.price }).map((_id, index) => (
                        <svg key={index} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6"
                        ><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                ...
                <h2 className="flex text-center">Duration: {itinerary.duration} days</h2>
                ...
                <p className="flex flex-wrap">
                    {Array.isArray(itinerary.hashtags)
                        ? itinerary.hashtags.map((tag, index) => (
                            <span key={index} className="bg-transparent border rounded-full border-blue-500 text-blue-500 px-2 py-1 m-1"> {tag}
                            </span>
                        )) : null}
                </p>
                <div className="">
                ...
                {expanded && (
                    <div className='flex flex-wrap justify-center col-span-4 gap-2'>
                        {itinerary.activities.map((activity) => (
                            <div key={activity} className="card w-96 bg-base-100 shadow-2xl">
                                <figure>
